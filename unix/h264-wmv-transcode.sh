...
# ===============================
# This script transcodes WMV video files to H.264 format using ffmpeg.
...
#
# USAGE:
#   ./h264-wmv-transcode.sh           # Process current directory only (software encoding)
#   ./h264-wmv-transcode.sh -r        # Process recursively from current directory
#   ./h264-wmv-transcode.sh -r -q     # Use Intel Quick Sync hardware acceleration
#   ./h264-wmv-transcode.sh -r -n     # Use NVIDIA NVENC hardware acceleration
#   ./h264-wmv-transcode.sh -r -a     # Use AMD AMF hardware acceleration
#
# OUTPUT:
...
if [[ "$RECURSE" == true ]]; then
    ...
    if [[ $total_files -eq 0 ]]; then
        echo "No eligible WMV files found to process."
        exit 0
    fi
