...
# - Transcodes .mpg files to H.264 (libx264 codec, or hardware accel if requested)
...
#
# USAGE:
#   ./h264-mpg-transcode.sh           # Process current directory only (software encoding)
#   ./h264-mpg-transcode.sh -r        # Process recursively from current directory
#   ./h264-mpg-transcode.sh -r -q     # Use Intel Quick Sync hardware acceleration
#   ./h264-mpg-transcode.sh -r -n     # Use NVIDIA NVENC hardware acceleration
#   ./h264-mpg-transcode.sh -r -a     # Use AMD AMF hardware acceleration
#
# OUTPUT:
...
if [[ "$RECURSE" == true ]]; then
    ...
    while IFS= read -r -d '' f; do
        ...
        output=""
        base_name=$(basename "$f")
        [[ "${base_name,,}" == *_redu.mpg ]] && continue
        output="${f%.*}_REDU.mp4"
        [[ -e "$output" ]] && continue
        files_to_process+=("$f")
    done < <(find . -type f -iname "*.mpg" -print0)

    total_files=${#files_to_process[@]}
    
    if [[ $total_files -eq 0 ]]; then
        echo "No eligible MPG files found to process."
        exit 0
    fi
