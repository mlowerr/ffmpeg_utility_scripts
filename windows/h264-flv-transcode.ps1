...
#
# USAGE:
#   .\h264-flv-transcode.ps1              # Process current directory only (software)
#   .\h264-flv-transcode.ps1 -Recurse     # Process recursively from current directory
#   .\h264-flv-transcode.ps1 -Recurse -UseQuickSync   # Use Intel Quick Sync
#   .\h264-flv-transcode.ps1 -Recurse -UseNVENC       # Use NVIDIA NVENC
#   .\h264-flv-transcode.ps1 -Recurse -UseAMF         # Use AMD AMF
#
# OUTPUT:
...
try {
    ...
    Get-ChildItem -File @recurseOption | Where-Object { 
        $_.Extension -ieq ".flv" -and $_.BaseName -notlike "*_REDU" 
    } | ForEach-Object {
    ...
    if ($totalFiles -eq 0) {
        Write-Host "No eligible FLV files found to process."
        exit 0
    }
