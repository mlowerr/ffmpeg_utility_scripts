...
#
# USAGE:
#   .\h264-wmv-transcode.ps1              # Process current directory only (software)
#   .\h264-wmv-transcode.ps1 -Recurse     # Process recursively from current directory
#   .\h264-wmv-transcode.ps1 -Recurse -UseQuickSync   # Use Intel Quick Sync
#   .\h264-wmv-transcode.ps1 -Recurse -UseNVENC       # Use NVIDIA NVENC
#   .\h264-wmv-transcode.ps1 -Recurse -UseAMF         # Use AMD AMF
#
# OUTPUT:
...
try {
    ...
    Get-ChildItem -File @recurseOption | Where-Object { 
        $_.Extension -ieq ".wmv" -and $_.BaseName -notlike "*_REDU" 
    } | ForEach-Object {
    ...
    if ($totalFiles -eq 0) {
        Write-Host "No eligible WMV files found to process."
        exit 0
    }
