...
# ======================================
# Converts .wav audio files in the current directory (or recursively with -Recurse)
...
function Rename-FilesWithSpaces {
    ...
    Get-ChildItem -LiteralPath $Directory.FullName -File -Filter "*.wav" | Where-Object { $_.Name -like "* *" } | ForEach-Object {
...
try {
    ...
    $fileParams = @{ LiteralPath = (Get-Location).Path; File = $true; Filter = "*.wav" }
    ...
    if ($filesToProcess.Count -eq 0) {
        Write-Host "No eligible WAV files found to process."
        exit 0
    }
